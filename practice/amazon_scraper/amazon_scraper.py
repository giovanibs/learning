...
def getPrice(result):
    try:
        ...
        return 'sem preço' if NO_PRICE else 0
...
records = []
page = 0
print(f'Pesquisando por \"{SEARCH_TERMS.replace(" ", "+")}\". Resultados sem preço {"" if NO_PRICE else "não "}incluídos')
while url:
    page += 1
    soup = getData(url)
...
print('Gravando resultados no Excel...')
